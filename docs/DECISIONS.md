...
## 2025-12-08 - Resend 邮件服务集成

### 1. 选择 Resend 作为邮件服务提供商

**决策**: 使用 Resend 而非 SendGrid/Mailgun/AWS SES

**原因**:
- 现代化 API 设计（React Email 支持）
- 免费额度充足（3,000 封/月）
- 简单的域名验证流程
- 优秀的开发者体验
- 详细的日志和监控
...
**实现**:
1. **团队邮件** (`noreply@livboss.com` → `hello@livboss.com`)
   - 完整客户信息
   - 留言内容
   - Reply-To 设置为客户邮箱

2. **客户自动回复** (`hello@livboss.com` → 客户)
   - 感谢信息
   - 24-48 小时回复承诺
   - WhatsApp 快速联系方式
...
- ContactEmailTemplate（团队）
- ContactAutoReplyTemplate（客户）
...
```bash
# 本地开发
.env.local: RESEND_API_KEY=re_xxxxx
...
**发件地址**:
- `noreply@livboss.com` - 系统通知
- `hello@livboss.com` - 客服回复
