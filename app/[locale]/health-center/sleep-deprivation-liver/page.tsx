...
export async function generateMetadata({ params }: Props) {
  ...
  const t = await getTranslations({ locale, namespace: 'health.articles.article2' });
  
  const siteUrl = process.env.NEXT_PUBLIC_SITE_URL || 'https://www.livboss.com';
  const localePath = locale === 'en' ? '' : `/${locale}`;
  const currentUrl = `${siteUrl}${localePath}/health-center/sleep-deprivation-liver`;
...
export default async function SleepDeprivationLiverArticle({ params }: Props) {
  const { locale } = await params;
